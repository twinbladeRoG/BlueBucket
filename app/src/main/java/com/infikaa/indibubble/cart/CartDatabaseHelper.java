...
/**
 * Created by Sudipta Saha on 07-07-2016.
 */
public class CartDatabaseHelper extends SQLiteOpenHelper {
    private static final String DATABASE_NAME = "Bluebucket.sqlite";
    private static final String DATABASE_CARTTABLE = "cart";
    private static final int DATABASE_VERSION = 2;

    private static CartDatabaseHelper mInstance = null;

    public static CartDatabaseHelper getInstance(Context ctx) {
        ...
        if (mInstance == null) {
            ...
            mInstance = new CartDatabaseHelper(ctx.getApplicationContext());
        }
        return mInstance;
    }
    ...
    private void createCart(SQLiteDatabase sqLiteDatabase) {
                ...
                sqLiteDatabase.execSQL("create table "+DATABASE_CARTTABLE+"( id integer primary key, picture text, brand text, name text, volume text, price text, offerdescription text, quantity text, postalcode text, description text,  distName text,  productid text unique  )");
    ...
    public boolean deleteItemByProductid(String productid){
        SQLiteDatabase db = this.getWritableDatabase();
        boolean success;
        success = db.delete(DATABASE_CARTTABLE,"productid = '"+productid+"'",null)>0;
        db.close();
        this.close();
        return success;
    }
